...
export class CandidateController {
  ...
  // Mobile-friendly job details including match percentage (token-based, extracts candidate ID from JWT if available)
  // GET /candidates/jobs/:jobId/mobile
  // Works with or without authentication - skips user-dependent activities if no token
  @Get('jobs/:jobId/mobile')
  @ApiOperation({ summary: 'Get mobile-optimized job details by ID (includes match percentage if authenticated)' })
  ...
  // @deprecated Use GET /candidates/jobs/:jobId/mobile instead (extracts candidate ID from JWT token)
  @Get(':id/jobs/:jobId/mobile')
  @ApiOperation({ 
    summary: 'Get mobile-optimized job details by ID (includes match percentage)',
    deprecated: true,
    description: 'DEPRECATED: Use GET /candidates/jobs/:jobId/mobile instead, which extracts candidate ID from JWT token'
  })
  @ApiParam({ name: 'id', description: 'Candidate ID', required: true })
  @ApiParam({ name: 'jobId', description: 'Job Posting ID', required: true })
  @ApiOkResponse({ description: 'Mobile job projection', type: MobileJobPostingDto })
